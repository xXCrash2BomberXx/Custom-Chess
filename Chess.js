...
class Piece {
        ...
        if (move.includes("(")) {
            for (let t = 0; t < move.length; t++) {
                if (move[t] == "." && move.slice(0, t).count("(") == move.slice(0, t).count(")")) {
                    var p1 = move.slice(0, t);
                    var p2 = move.slice(t + 1);
                    break;
                }
            }
        } else {
            var p1 = move.slice(0, move.indexOf("."));
            var p2 = move.slice(move.indexOf(".") + 1);
        }
    ...
    static #move(move, x1, y1, x2, y2, direction = 1, turns = 0, xLim = 8, yLim = 8, lxLim = 0, lyLim = 0) {
        ...
        } else if (move.includes("i") && turns > 0) {
            return false;
        // Orthogonally (+)
        } else if (move.includes("+") && !((x1 != x2 && y1 == y2) || (y1 != y2 && x1 == x2))) {
            return false;
        //Diagonally (x, X)
        } else if (move.includes("x") && !(Math.abs(x1 - x2) == Math.abs(y1 - y2))) {
            return false;
        // Orthogonally or Diagonally (*)
        } else if ((move.includes("*") || (!move.includes("+") && !move.includes("x") && !move.includes("/"))) && !(((x1 != x2 && y1 == y2) || (y1 != y2 && x1 == x2)) || Math.abs(x1 - x2) == Math.abs(y1 - y2))) {
            return false;
        // Forward (>, >=, >r, >l)
        ...
        // Backward (<, <=, <r, <l)
            ...
            return false;
        // Forward or Backward (<>)
        } else if (move.includes(">") && move.includes("<") && y1 == y2) {
            return false;
        // Orthogonally Sideways (=)
        } else if (!move.includes(">") && !move.includes("<") && move.includes("=") && y1 != y2) {
            return false;
    ...
    test () {
        if (this.y == (this.direction == 1 ? this.yLim-1 : 0)) {
            ...
            this.plot = function (canvas, xSquares = 8, ySquares = 8) {
                ...
                ctx.fillText("\u{2654}", canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.9));
                return null;
            }
            return true;
        }
...
class King extends Piece {
    ...
    plot(canvas, xSquares = 8, ySquares = 8) {
        ...
        ctx.fillStyle = this.color;
        ctx.font = String(Math.min(canvas.width/this.xLim, canvas.height/this.yLim))+"px sans-serif";
        ctx.fillText((this.direction == 1?"\u{2654}":"\u{265A}"), canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.8));
    }
}
...
class Queen extends Piece {
    ...
    plot(canvas, xSquares = 8, ySquares = 8) {
        ...
        ctx.fillStyle = this.color;
        ctx.font = String(Math.min(canvas.width/this.xLim, canvas.height/this.yLim))+"px sans-serif";
        ctx.fillText((this.direction == 1?"\u{2655}":"\u{265B}"), canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.8));
    }
}
...
class Rook extends Piece {
    ...
    plot(canvas, xSquares = 8, ySquares = 8) {
        ...
        ctx.fillStyle = this.color;
        ctx.font = String(Math.min(canvas.width/this.xLim, canvas.height/this.yLim))+"px sans-serif";
        ctx.fillText((this.direction == 1?"\u{2656}":"\u{265C}"), canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.8));
    }
}
...
class Bishop extends Piece {
    ...
    plot(canvas, xSquares = 8, ySquares = 8) {
        ...
        ctx.fillStyle = this.color;
        ctx.font = String(Math.min(canvas.width/this.xLim, canvas.height/this.yLim))+"px sans-serif";
        ctx.fillText((this.direction == 1?"\u{2657}":"\u{265D}"), canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.8));
    }
}
...
class Knight extends Piece {
    ...
    plot(canvas, xSquares = 8, ySquares = 8) {
        ...
        ctx.fillStyle = this.color;
        ctx.font = String(Math.min(canvas.width/this.xLim, canvas.height/this.yLim))+"px sans-serif";
        ctx.fillText((this.direction == 1?"\u{2658}":"\u{265E}"), canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.8));
    }
}
...
class Pawn extends Piece {
    ...
    plot(canvas, xSquares = 8, ySquares = 8) {
        ...
        ctx.fillStyle = this.color;
        ctx.font = String(Math.min(canvas.width/this.xLim, canvas.height/this.yLim))+"px sans-serif";
        ctx.fillText((this.direction == 1?"\u{2659}":"\u{265F}"), canvas.width/this.xLim*(this.x+0.5), canvas.height/this.yLim*(this.y+0.8));
    }
