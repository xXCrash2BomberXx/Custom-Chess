...
class Piece {
    ...
    static #rider(move, x1, y1, x2, y2, direction = 1, turns = 0, xLim = 8, yLim = 8, lxLim = 0, lyLim = 0) {
        ...
        if (factor.match(/[0-9]+$/)) {
            if (Math.abs(x2 - x1) % parseInt(factor.match(/[0-9]+$/g)[0]) == 0 && Math.abs(y2 - y1) % parseInt(factor.match(/[0-9]+$/g)[0]) == 0) {
                ...
                for (let i = 0; i < parseInt(factor.match(/[0-9]+$/g)[0]); i++) {
                    if (pattern.includes("(") && pattern.includes(".")) {
                        ...
                        for (let t = 0; t < pattern.length; t++)
                            if (pattern[t] == "." && (pattern.slice(0, t).match(/\(/g) || []).length == (pattern.slice(0, t).match(/\)/g) || []).length) {
                                temp2 = Piece.#then(factor.slice(0, factor.match(/[0-9]+$/).index) +
                                    pattern.slice(0, t) + "." +
                                    factor.slice(0, factor.match(/[0-9]+$/)) + pattern.slice(t + 1), x1 + i * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + i * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), x1 + (i + 1) * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + (i + 1) * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), direction, turns, xLim, yLim, lxLim, lyLim);
                                break;
                            }
                            else if (t == pattern.length - 1) {
                                temp2 = Piece.#rider(factor.slice(0, factor.match(/[0-9]+$/).index) + pattern, x1 + i * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + i * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), x1 + (i + 1) * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + (i + 1) * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), direction, turns, xLim, yLim, lxLim, lyLim);
                            }
                        if (temp2)
                            temp.push(...temp2);
                        else
                        ...
                        let temp2 = Piece.#rider(factor.slice(0, factor.match(/[0-9]+$/).index) + pattern, x1 + i * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + i * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), x1 + (i + 1) * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + (i + 1) * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), direction, turns, xLim, yLim, lxLim, lyLim);
                        if (temp2)
                            temp.push(...temp2);
                        else
                            return false;
                        ...
                        let temp2 = Piece.#then(factor.slice(0, factor.match(/[0-9]+$/).index) +
                            ...
                            factor.slice(0, factor.match(/[0-9]+$/).index) + pattern.slice(pattern.indexOf(".") + 1), x1 + i * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + i * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), x1 + (i + 1) * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + (i + 1) * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), direction, turns, xLim, yLim, lxLim, lyLim);
                        if (temp2)
                            temp.push(...temp2);
                        else
                            return false;
                    }
                    else if (Piece.#move(factor.slice(0, factor.match(/[0-9]+$/).index) + pattern, x1 + i * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + i * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), x1 + (i + 1) * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + (i + 1) * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0]), direction, turns, xLim, yLim, lxLim, lyLim))
                        temp.push([x1 + (i + 1) * (x2 - x1) / parseInt(factor.match(/[0-9]+$/g)[0]), y1 + (i + 1) * (y2 - y1) / parseInt(factor.match(/[0-9]+$/g)[0])]);
                    else
                        return false;
                }
                return temp;
            ...
            for (let i = 1; i < Math.max(Math.abs(x2 - x1), Math.abs(y2 - y1)); i++) {
                temp = Piece.#rider(factor.slice(0, factor.match(/[nN]$/).index) + String(i) + move.slice(move.indexOf("("), move.indexOf(")") + 1), x1, y1, x2, y2, direction, turns, xLim, yLim, lxLim, lyLim);
                if (temp)
                    return temp;
            }
            return false;
    ...
    static #move(move, x1, y1, x2, y2, direction = 1, turns = 0, xLim = 8, yLim = 8, lxLim = 0, lyLim = 0) {
        ...
        let match3;
        if (match2)
            match3 = match2[0].match(/[0-9nN]+(-[0-9nN]+)?/g); //.match(/[0-9n]+/g);
        // In X-Range
        if (x2 >= xLim || x2 < lxLim)
            return false;
        // In Y-Range
        ...
        if (!match2 &&
            match1 &&
            (match1[0].includes("-") ?
                !((range(parseInt(match1[0].match(/[0-9n]+/g)[0]), parseInt(match1[0].match(/[0-9n]+/g)[1]) + 1).includes(Math.abs(x1 - x2)) && (Math.abs(x1 - x2) == Math.abs(y1 - y2) || y1 == y2)) ||
                    (range(parseInt(match1[0].match(/[0-9n]+/g)[0]), parseInt(match1[0].match(/[0-9n]+/g)[1]) + 1).includes(Math.abs(y1 - y2)) && (Math.abs(y1 - y2) == Math.abs(x1 - x2) || x1 == x2))) :
        ...
        if (match2 && match3 &&
            !(((match3[0].includes("-") ?
                range(parseInt(match3[0].match(/[0-9n]+/g)[0]), parseInt(match3[0].match(/[0-9n]+/g)[1]) + 1).includes(Math.abs(x1 - x2)) :
                ((match3[0] != "n" && parseInt(match3[0]) == Math.abs(x1 - x2)) || match3[0] == "n")) &&
                (match3[1].includes("-") ?
                    range(parseInt(match3[1].match(/[0-9n]+/g)[0]), parseInt(match3[1].match(/[0-9n]+/g)[1]) + 1).includes(Math.abs(y1 - y2)) :
                    ((match3[1] != "n" && parseInt(match3[1]) == Math.abs(y1 - y2)) || match3[1] == "n"))) ||
                (((match3[0].includes("-") ?
                    range(parseInt(match3[0].match(/[0-9n]+/g)[0]), parseInt(match3[0].match(/[0-9n]+/g)[1]) + 1).includes(Math.abs(y1 - y2)) :
                    ((match3[0] != "n" && parseInt(match3[0]) == Math.abs(y1 - y2)) || match3[0] == "n")) &&
                    (match3[1].includes("-") ?
                        range(parseInt(match3[1].match(/[0-9n]+/g)[0]), parseInt(match3[1].match(/[0-9n]+/g)[1]) + 1).includes(Math.abs(x1 - x2)) :
    ...
    static move(moves, x1, y1, x2, y2, direction = 1, turns = 0, xLim = 8, yLim = 8, lxLim = 0, lyLim = 0, others = []) {
        ...
        for (let i = 0; i < moves.length; i++) {
            // Leaper (~)
            if (!moves[i].includes("~") && !moves[i].includes("^") && Piece.path(x1, y1, x2, y2).slice(0, -1).filter((value) => Piece.getPiece(value[0], value[1], others)).length != 0)
                continue;
            // Locust (^)
            if (moves[i].includes("^") && Piece.path(x1, y1, x2, y2, 2).some((arr) => {
            ...
            if (moves[i].includes("(") || moves[i].includes(".")) {
                ...
                if (moves[i].includes("(") && moves[i].includes("."))
                    ...
                    step = Piece.#then(moves[i].replaceAll("^", "~"), x1, y1, x2, y2, direction, turns, xLim, yLim, lxLim, lyLim);
                if (!step || (moves[i].includes("^") ? step : step.slice(0, -1)).filter((value) => Piece.getPiece(value[0], value[1], others)).length != 0)
                    continue;
                else 
                // Return success
            ...
            if (moves[i].includes("^")) {
                ...
                for (let i = 0; i < pieces.length; i++)
                    pieces[i] = Piece.getPiece(pieces[i][0], pieces[i][1], others);
                if (pieces.length == 0)
                    continue;
...
class Pawn extends Piece {
    ...
    getMoves(others = []) {
        ...
        let temp = Piece.getPiece(this.x - this.direction, this.y, others);
        if (temp && temp.constructor.name == "Pawn" && temp.turns == 1 && temp.enPassant &&
